...
cluster_report = load_module(
    ...
    "scripts/identity_construction_cluster_report.py",
)
...
def make_record(stem: str, **overrides: object) -> dict[str, object]:
    data: dict[str, object] = {
        ...
        "related_terms": [],
        "example_phrases": [{"pali": stem, "translation": f"{stem} translation", "source": "SN 22.89"}],
        "sutta_references": ["SN 22.89"],
        "tags": ["self-view"],
        "authority_basis": [
...
class ClusterReportTests(unittest.TestCase):
    def test_build_report_flags_missing_formula_records(self) -> None:
        ...
        self.assertIn("sakkayassa-samudayo", report["errors"]["missing_formula_records"])
    ...
    def test_build_report_warns_on_preferred_rendering_mismatch(self) -> None:
        ...
        terms["asmimana"]["preferred_translation"] = "ego"

        report = cluster_report.build_report(terms)

        self.assertIn("asmimana", report["warnings"]["preferred_rendering_mismatches"])
    ...
    def test_write_outputs_creates_expected_files(self) -> None:
        ...
        self.assertTrue(any(path.name == "identity-construction-cluster-glossary.md" for path in written))
    ...
    def test_main_emits_json(self) -> None:
        ...
        with mock.patch.object(cluster_report, "load_terms", return_value={}):
            with mock.patch.object(cluster_report, "build_report", return_value=fake_report):
                with mock.patch("sys.argv", ["identity_construction_cluster_report.py", "--format", "json"]):

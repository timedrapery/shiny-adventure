...
cluster_report = load_module(
    ...
    "scripts/consummation_interface_cluster_report.py",
)
...
class ConsummationInterfaceClusterReportTests(unittest.TestCase):
    def test_build_report_flags_missing_formula_records(self) -> None:
        ...
        self.assertIn("ditthadhammanibbana", report["errors"]["missing_formula_records"])
    ...
    def test_build_report_warns_on_preferred_rendering_mismatch(self) -> None:
        ...
        terms["amata"]["preferred_translation"] = "immortal"

        report = cluster_report.build_report(terms)

        self.assertIn("amata", report["warnings"]["preferred_rendering_mismatches"])
    ...
    def test_write_outputs_creates_expected_files(self) -> None:
        ...
        self.assertTrue(any(path.name == "consummation-interface-glossary.md" for path in written))

    def test_render_contrast_sheet_mentions_vimutti_and_yogakkhema(self) -> None:
        terms = {
            stem: make_record(stem)
            for stem in cluster_report.HEADWORD_TERMS + cluster_report.SUPPORTING_TERMS + cluster_report.FORMULA_TERMS
        }
        ...
        self.assertIn("yogakkhema", sheet)
    ...
    def test_main_emits_json(self) -> None:
        ...
        with mock.patch.object(cluster_report, "load_terms", return_value={}):
            with mock.patch.object(cluster_report, "build_report", return_value=fake_report):
                with mock.patch("sys.argv", ["consummation_interface_cluster_report.py", "--format", "json"]):

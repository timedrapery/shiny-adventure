...
cluster_report = load_module("knowledge_cluster_report", "scripts/knowledge_cluster_report.py")
...
def make_record(stem: str, **overrides: object) -> dict[str, object]:
    data: dict[str, object] = {
        ...
        "related_terms": [],
        "example_phrases": [{"pali": stem, "translation": f"{stem} translation", "source": "MN 26"}],
        "sutta_references": ["MN 26"],
        "tags": ["knowledge-cluster"],
        "authority_basis": [
...
class KnowledgeClusterReportTests(unittest.TestCase):
    def test_build_report_flags_missing_headwords(self) -> None:
        ...
        self.assertIn("nana", report["errors"]["missing_headwords"])
    ...
    def test_build_report_warns_on_preferred_rendering_mismatch(self) -> None:
        ...
        terms["nana"]["preferred_translation"] = "knowing"

        report = cluster_report.build_report(terms)

        self.assertIn("nana", report["warnings"]["preferred_rendering_mismatches"])
    ...
    def test_write_outputs_creates_expected_files(self) -> None:
        ...
        self.assertTrue(any(path.name == "knowledge-cluster-glossary.md" for path in written))

    def test_render_contrast_sheet_mentions_sati_and_sampajanna(self) -> None:
        terms = {
            stem: make_record(stem)
            for stem in cluster_report.HEADWORD_TERMS + cluster_report.SUPPORTING_TERMS
        }
        ...
        self.assertIn("sati", sheet)
        self.assertIn("sampajañña", sheet)
    ...
    def test_main_emits_json(self) -> None:
        ...
        with mock.patch.object(cluster_report, "load_terms", return_value={}):
            with mock.patch.object(cluster_report, "build_report", return_value=fake_report):
                with mock.patch("sys.argv", ["knowledge_cluster_report.py", "--format", "json"]):

...
cluster_report = load_module(
    "kama_cluster_report",
    "scripts/kama_cluster_report.py",
)
...
def make_record(stem: str, **overrides: object) -> dict[str, object]:
    data: dict[str, object] = {
        ...
        "related_terms": [],
        "example_phrases": [{"pali": stem, "translation": f"{stem} translation", "source": "MN 13"}],
        "sutta_references": ["MN 13"],
        "tags": ["core-doctrine"],
        "authority_basis": [
...
class KamaClusterReportTests(unittest.TestCase):
    def test_build_report_flags_missing_headwords(self) -> None:
        ...
        self.assertIn("kama", report["errors"]["missing_headwords"])
    ...
    def test_build_report_warns_on_preferred_rendering_mismatch(self) -> None:
        ...
        terms["kamacchanda"]["preferred_translation"] = "desire"

        report = cluster_report.build_report(terms)

        self.assertIn("kamacchanda", report["warnings"]["preferred_rendering_mismatches"])
    ...
    def test_write_outputs_creates_expected_files(self) -> None:
        ...
        self.assertTrue(any(path.name == "kama-cluster-glossary.md" for path in written))
    ...
    def test_main_emits_json(self) -> None:
        ...
        with mock.patch.object(cluster_report, "load_terms", return_value={}):
            with mock.patch.object(cluster_report, "build_report", return_value=fake_report):
                with mock.patch("sys.argv", ["kama_cluster_report.py", "--format", "json"]):

...
cluster_report = load_module(
    ...
    "scripts/five_heaps_cluster_report.py",
)
...
def make_record(stem: str, **overrides: object) -> dict[str, object]:
    data: dict[str, object] = {
        ...
        "related_terms": [],
        "example_phrases": [{"pali": stem, "translation": f"{stem} translation", "source": "SN 22.48"}],
        "sutta_references": ["SN 22.48"],
        "tags": ["aggregates"],
        "authority_basis": [
...
class ClusterReportTests(unittest.TestCase):
    def test_build_report_flags_missing_formula_records(self) -> None:
        terms = {
            ...
            for stem in cluster_report.HEADWORD_TERMS + cluster_report.COLLECTIVE_TERMS
        }
    ...
    def test_build_report_warns_on_collective_drift(self) -> None:
        terms = {
            stem: make_record(stem)
            for stem in cluster_report.HEADWORD_TERMS + cluster_report.COLLECTIVE_TERMS + cluster_report.FORMULA_TERMS
        }
        terms["pancupadanakkhandha"]["preferred_translation"] = "five heaps of clinging"

        report = cluster_report.build_report(terms)

        self.assertIn(
            "pancupadanakkhandha",
            report["warnings"]["inconsistent_clung_to_heap_renderings"],
        )
    ...
    def test_write_outputs_creates_expected_files(self) -> None:
        ...
        self.assertTrue(any(path.name == "five-heaps-formula-sheet.md" for path in written))
    ...
    def test_main_emits_json(self) -> None:
        ...
        fake_report = {
            ...
            "warnings": {
                ...
                "inconsistent_clung_to_heap_renderings": [],
            },
        }
        ...
        with mock.patch.object(cluster_report, "load_terms", return_value={}):
            with mock.patch.object(cluster_report, "build_report", return_value=fake_report):
                with mock.patch("sys.argv", ["five_heaps_cluster_report.py", "--format", "json"]):

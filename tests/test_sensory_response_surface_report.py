...
def make_record(stem: str, **overrides: object) -> dict[str, object]:
    data: dict[str, object] = {
        ...
        "example_phrases": [{"pali": stem, "translation": f"{stem} translation", "source": "MN 137"}],
        "sutta_references": ["MN 137"],
        "tags": ["core-practice"],
        "authority_basis": [{"source": "source-a", "scope": "scope-a"}],
        "translation_policy": {
...
class SensoryResponseSurfaceReportTests(unittest.TestCase):
    def test_build_report_flags_missing_control_records(self) -> None:
        ...
        self.assertIn("gehasita-somanassa", report["errors"]["missing_control_records"])
    ...
    def test_build_report_flags_missing_translation_control_line(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mn148_path = repo_root / "docs/translations/mn148-chachakka-sutta.md"
            mn148_path.write_text(
                "when one is touched by pleasant feeling, one delights in it, affirms it, and keeps taking it personally\n",
                encoding="utf-8",
            )
        ...
        self.assertTrue(
            any(
                "mn148-painful-feeling-trained-response" in item
    ...
    def test_write_outputs_creates_expected_file(self) -> None:
        ...
        self.assertEqual(written[0].name, "sensory-response-control-sheet.md")
    ...
    def test_main_emits_json(self) -> None:
        ...
        with mock.patch.object(surface_report, "load_terms", return_value={}):
            with mock.patch.object(surface_report, "build_report", return_value=fake_report):
                with mock.patch("sys.argv", ["sensory_response_surface_report.py", "--format", "json"]):

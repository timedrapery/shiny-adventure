# OSF Pali Readings

Early Buddhist discourses in plain, contemporary English.

All 64 translations in this collection are readable here. Each one is backed by a governed Pali lexicon, so a term that matters is translated the same way every time it appears, and every translation decision has a written reason behind it. See [About this translation](about.md) for what that means in practice.

## Start with one short teaching

No Pali and no Buddhist background are required. Start with a human question rather than a vocabulary lesson.

[Read **Good Friendship Is the Whole Path** — about 2 minutes](suttas/sn45-2-upaddha-sutta.md){ .md-button .md-button--primary }

## Choose what brings you here

<div class="newcomer-pathways" markdown="1">

<section class="newcomer-pathway" markdown="1">

### I am curious but skeptical

Begin with how to evaluate a teaching and why some questions are set aside.

About 22 minutes total:

- [**How to Test a Teaching**](suttas/an3-65-kesamutta-sutta.md)
- [**The Man Struck by a Poisoned Arrow**](suttas/mn63-culamalukya-sutta.md)
- [**Good Friendship Is the Whole Path**](suttas/sn45-2-upaddha-sutta.md)

</section>

<section class="newcomer-pathway" markdown="1">

### I am dealing with stress or pain

Start with pain, changing circumstances, and the practical meaning of ignorance.

About 10 minutes total:

- [**One Arrow, Not Two**](suttas/sn36-6-salla-sutta.md)
- [**When Life Goes Up and Down**](suttas/an8-6-dutiyalokadhamma-sutta.md)
- [**What Ignorance Means**](suttas/sn56-17-avijja-sutta.md)

</section>

<section class="newcomer-pathway" markdown="1">

### I want something practical to try

Begin with ethical awareness, everyday recollection, and training thought.

About 14 minutes total:

- [**What Keeps the World Human**](suttas/an2-9-cariya-sutta.md)
- [**Six Things to Remember Anywhere**](suttas/an11-12-dutiyamahanama-sutta.md)
- [**Two Kinds of Thinking**](suttas/mn19-dvedhavitakka-sutta.md)

</section>

</div>

## Go further

- [**Your First 12**](start-here.md#your-first-12) — a compact route from short, recognizable teachings into the foundations
- [**Complete reading order**](start-here.md#the-complete-reading-order) — all 64 texts, arranged for a newcomer
- [**All Suttas**](suttas/index.md) — the full collection, if you already know what you are looking for
- [**Find a sutta**](find-a-sutta.md) — filter by topic, difficulty, form, or reading length
- [**Glossary**](glossary.md) — the recurring vocabulary, in plain English
- <a href="downloads/osf-pali-readings.epub"><strong>Download the whole collection</strong></a> — EPUB, for e-readers, phones, and reading offline

## How to read these

The translations use clear, common English. Each page gives a reading time and a short orientation before the governed translation. Where a word is doing technical work, open the words-used panel for a plain-English definition. Some browsers also show the same note when you point to a marked word.

Repetition has been kept where the texts repeat. These were spoken and memorised, and the repeating is part of how they work.

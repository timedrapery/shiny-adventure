...
- This is a repo-native study translation of AN 3.65, designed to be readable
  as a single Markdown document.
- Companion translator notes:
  [AN 3.65 Translation Notes](an3-65-kesamutta-sutta-notes.md)
- Source basis: the Pali text of AN 3.65 as segmented in SuttaCentral's
  Bilara data. The sutta is titled `Kesamuttisutta` in the Bilara (after the
  town of Kesamutta where it was delivered); it is also widely known as the
  Kālāma Sutta (after the people addressed).
- House choices that most affect this sutta: `lobha` is rendered `greed`;
  `dosa` is rendered `aversion`; `moha` is rendered `delusion`; `kusala` is
...
- This edition treats AN 3.65 as the repository's control surface for the
  ...
  insufficient for accepting or rejecting a teaching — and for the four
  consolations that arise from practice free of greed, aversion, and delusion,
  regardless of one's views about the afterlife.

<!--
  ...
  ordinary, non-technical prose on the same page -- the abbr extension
  tooltips every literal occurrence, so a too-common word (e.g. bare "arrow"
  ...
  pages -- do not write a page-specific definition for a word that recurs
  elsewhere with a different sense. Check for this before adding a new entry.
-->
...
<!--
  ...
  multi-word governed renderings appeared in the reader with no tooltip at
  all, across 3,372 uses.

  Entries below are the tier appearing on five or more pages. They gloss the
  BASE phrase, not every compound built on it -- python-markdown's abbr
  matches longest-first, so "first mental theme" would win over "mental
  theme" if both were defined, and defining only the base keeps the file
  maintainable while still tooltipping the compound's core.
  ---------------------------------------------------------------------------
-->
...
<!--
  Second pass, same date: the words a general reader is most likely to stall
  on, which the first pass missed because they are single words rather than
  multi-word coinages.

  Two classes. (1) Pali the project deliberately leaves untranslated
  (STYLE_GUIDE.md) -- "bhikkhus" alone appears on 35 of 44 pages and was
  never glossed anywhere. (2) Ordinary English words doing technical work,
  which are more dangerous than obvious jargon precisely because the reader
  does not notice they have missed something.

  NOTE: python-markdown's abbr is case-sensitive and does not match plurals,
  so each surface form needs its own entry ("bhikkhu" does not cover
  "bhikkhus", "bhante" does not cover "Bhante").

  "becoming" was deliberately NOT glossed. It is bhava throughout the
  dependent-arising material, but the psychic-powers passage uses it as an
  ordinary participle ("becoming one from many"), and no single gloss is true
  of both. Per the rule above, skipped rather than fudged.
-->

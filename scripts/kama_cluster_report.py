#!/usr/bin/env python3
"""Audit and generate translator-facing outputs for the kama / sensuality cluster."""
...
def render_mismatch_warnings(terms: dict[str, dict[str, object]]) -> list[str]:
    ...
    expected = {
        ...
        "kamupadana": "taking sensuality personally",
        "kamabhava": "sensual existence",
...
def render_contrast_sheet(terms: dict[str, dict[str, object]]) -> str:
    lines = [
        "# Kāma Cluster Contrast Sheet",
        "",
        f"- `{terms['kama']['term']}`: `{terms['kama']['preferred_translation']}`",
        f"- `{terms['kamacchanda']['term']}`: `{terms['kamacchanda']['preferred_translation']}`",
        f"- `{terms['kama-tanha']['term']}`: `{terms['kama-tanha']['preferred_translation']}`",
        f"- `{terms['kama-raga']['term']}`: `{terms['kama-raga']['preferred_translation']}`",
        f"- `{terms['kamupadana']['term']}`: `{terms['kamupadana']['preferred_translation']}`",
        "",
        "## Keep Distinct",
        "",
        "- `kāma` is the sensual domain, not generic desire and not automatically sexual.",
        ...
        "",
        f"- `{terms['kamesu-micchacara']['term']}` -> `{terms['kamesu-micchacara']['preferred_translation']}` is a narrower ethical expression and should not control the default for bare `kāma`.",
    ]
    return "\n".join(lines)
...
def write_outputs(terms: dict[str, dict[str, object]]) -> list[Path]:
    ...
    outputs = {
        OUTPUT_DIR / "kama-cluster-glossary.md": render_glossary(terms),
        OUTPUT_DIR / "kama-cluster-contrast-sheet.md": render_contrast_sheet(terms),
    }

...
- `python scripts/read_aloud_kit.py --surface sn36_6`
  Generates the session kit for a read-aloud review: the current `body_sha256`, the translation split into numbered sentences so a stumble is recorded as a number rather than a description, and a ledger-ready record. Defaults to the three-text pilot. `--facilitator` appends the measured watch points from the spoken register profile; withhold that section until after the read. `--out` writes one file per surface.
